...
- `POST /care-event` takes `{"token": "...", "id": "client-generated-id", "type": "feed" | "clean" | "pet" | "play", "hits": <number, play only>}` and broadcasts it to every connected client over the existing `/ws` connection as `{"type": "care-event", "id": ..., "eventType": ..., "hits": ..., "sentAt": ...}`.
