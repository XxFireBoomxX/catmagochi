...
- `src/hooks/useNotificationPermission.ts` — owns the OS permission prompt, and nothing else. **It is deliberately separate from `usePushSubscription`, and that separation is the fix for a real bug**: the prompt used to live inside that hook, which returns early when `VITE_RELAY_URL`/`VITE_RELAY_TOKEN`/`VITE_VAPID_PUBLIC_KEY` aren't all set. That made permission — and therefore *every* notification, including the local `attention` one documented as needing no server — silently conditional on relay configuration. Turning the setting on appeared to work and then nothing ever happened, with no error anywhere. Permission is a browser concern; push delivery is a relay concern; they are separate hooks because they are separate problems. Asks once per session (browsers only prompt once per origin; asking again after an answer is silently rejected).
...
- `src/hooks/useAttentionNotifications.ts` — the local-only `attention` type. Prefers the service worker's `showNotification` (it survives the app being backgrounded, which is most of when this matters) but **falls back to a page-level `new Notification()` when no worker is registered**. `vite-plugin-pwa` only emits a worker on build, so in `npm run dev` there is none at all and `serviceWorker.ready` never resolves — this alert could previously neither fire nor be tested locally. Watches `stats`/`sleeping`/`settings`, reuses `usePet`'s own critical-stat priority order (fullness > energy > cleanliness > happiness, matching `deriveMood`), and rate-limits itself to one notification per 30 minutes (`MIN_NOTIFY_INTERVAL_MS`, tracked via a ref) so it doesn't spam on every 5s tick while a stat stays low.
- `src/components/Menu.tsx`'s `settings` view surfaces the *permission* state before the push status — without permission nothing fires at all, so a push message underneath would describe a problem the user can't reach yet. It also says plainly when only `attention` is working because no relay is configured, rather than leaving the toggle reading `[ON]` beside silence. A global toggle plus three per-type toggles (each `disabled` while global is off, but still reflecting their own stored value underneath). Shows `PUSH_STATUS_TEXT[pushStatus]` for `unsupported`/`denied`/`error` states (nothing shown for `idle`/`subscribed`/`unsubscribed` — those aren't actionable by the user).
