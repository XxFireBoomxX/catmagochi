...
- ~~`[PLAY]` is a daily trick lesson.~~ Replaced after one release. The brief changed from "a small daily ritual" to a full RPG: skills, levels, loot and a cat whose look changes. `[PLAY]` is now the hunt — turn-based combat against mice, grasshoppers and worse, with XP and unlocking skills. Slices 1 and 2 of four are in (combat and levels; loot, bag, consumables and trinkets). Still to come: equipment drawn on the cat and a palette by level (3), and three more zones (4). See `docs/superpowers/specs/2026-09-03-hunt-rpg-design.md`.

- ~~`[PLAY]` opens a nudge picker that sends a canned line to the other device.~~ Replaced. The relay was never deployed and isn't planned, so `send()` was a no-op: `[PLAY]` walked you through a menu to send a note to nobody, then applied a flat stat reward. It's now a daily trick lesson — one a day, her mood plus your approach shift the odds, and once the lesson is spent the panel becomes a place to ask for tricks she already knows. Fully offline; the lesson still fires the same `play` care event, so it syncs unchanged if a relay ever appears. See `docs/superpowers/specs/2026-09-02-daily-trick-training-design.md` and CLAUDE.md's "Play (a daily trick lesson)".
